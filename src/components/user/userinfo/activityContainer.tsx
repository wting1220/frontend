...
import { secondMenu, userTagsShow } from '../../../utils/commen'
import { useLocation, useHistory } from 'react-router'

export default memo(function ActivityContainer({ nav }: any) {

  const [shareMenu, setShareMenu] = useState<string>('hot') // 二级菜单栏绑定
  ...
  // useEffect(() => {
  //   setShareMenu(location.pathname.split('/')[3])
  // }, [location])

  return (
    <div className='activity-container'>
      {
        ['activity', 'share', 'like', 'collection', 'attention'].includes(nav) &&
        <div className='activity-container-title'>
          {
            secondMenu.map((item: any) => {
              return item.key === nav
                ? <>
                  <div><span className='title-left'>{item.label}  </span>{nav === 'activity' && <span>赞了这篇文章</span>}</div>
                  <div>
                    <Menu onClick={handleShareMenu} selectedKeys={[shareMenu]} mode="horizontal">
                      {item.child && item.child.map((child: any) => (<Menu.Item key={child.key}>{child.label}</Menu.Item>))}
                    </Menu>
                  </div>
                </>
                : null
      ...
      <ul>
        {
          ['activity', 'topic', 'like', 'collection'].includes(nav) ? lists.map(item => {
            return <li key={item.id} className='activity-container-topic'><TopicShow item={item} /></li>
          })
          : null
        }
        ...
        {
          nav === 'attention' && lists.map((item:any) => (userTagsShow(item, 'user')))
        }
      </ul>
    </div >
  )
